 ...
 */
export async function ensurePage(session, config) {
 ...
 */
export async function openAndCollect(session, config, url) {
...
export function refSelector(ref) {
    ...
    if (match === null)
        throw new Error(`invalid element ref "${ref}": expected @N from browser_open`);
