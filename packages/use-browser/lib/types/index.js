...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const text = await page.evaluate(`(function(){var root=${args.selector ? `document.querySelector(${JSON.stringify(args.selector)})` : 'document.body'};if(!root)return '';var t=(root.innerText||root.textContent||'').trim();return t.slice(0,${max});})()`);
            return { text };
        },
    }));
    ctx.tools.register(defineTool({
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, _exec) {
            ...
            return { action: 'type', ref: args.ref };
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_eval',
        ...
        async execute(args, _exec) {
            const cfg = current();
            const page = await ensurePage(session, cfg);
            const raw = await page.evaluate(args.expression);
            let serialized;
            try {
