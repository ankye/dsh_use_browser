...
*/
async function ensurePage(session, config) {
	if (session.page !== void 0) return session.page;
...
*/
async function openAndCollect(session, config, url) {
...
const Config = Schema.object({
	...
	cdpUrl: Schema.string().default("http://localhost:9222"),
	headless: Schema.boolean().default(true),
	timeoutMs: Schema.number().step(1).min(1e3).default(3e4)
});
...
function apply(ctx, config) {
	...
	ctx.systemPrompt.section({
		...
		order: 122,
		text: "To operate a browser: call browser_open(url) to see the page and its interactive @e refs, then browser_click / browser_type on a ref, browser_extract for text, browser_screenshot to save a PNG (then analyze_image on the path to see it), and browser_eval for page JS. The backend is selected by Settings → browser (playwright headless, or cdp to your own Chrome)."
	});
	ctx.tools.register(defineTool({
		name: "browser_open",
		description: "Open a URL in the browser (Playwright headless or your CDP Chrome per settings) and return the page title, URL, and a list of interactive elements as @e refs (e.g. @1, @2) for browser_click / browser_type.",
		parameters: {
			url: {
				type: "string",
				required: true,
			...
			timeoutMs: {
				type: "number",
				description: "Navigation timeout in ms; defaults to the configured value."
			}
		},
		output: {
			schema: {
		...
		async execute(args, _exec) {
			return await openAndCollect(session, withTimeout(current(), args.timeoutMs), args.url);
		}
	}));
	ctx.tools.register(defineTool({
		...
		parameters: {
			selector: {
				type: "string",
				description: "CSS selector; defaults to the whole page body."
			},
			maxChars: {
				type: "number",
		...
		async execute(args, exec) {
			...
			const page = await ensurePage(session, current());
			const max = args.maxChars ?? 8e3;
			return { text: await page.evaluate(`(function(){var root=${args.selector ? `document.querySelector(${JSON.stringify(args.selector)})` : "document.body"};if(!root)return '';var t=(root.innerText||root.textContent||'').trim();return t.slice(0,${max});})()`) };
		}
	}));
	ctx.tools.register(defineTool({
		...
		parameters: {
			path: {
				type: "string",
				description: "Output PNG path; defaults to a temp path."
			},
			fullPage: {
				type: "boolean",
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: `Screenshot saved: ${value.path}`
			}]
		},
		async execute(args, _exec) {
			const page = await ensurePage(session, current());
	...
	ctx.tools.register(defineTool({
		...
		parameters: { ref: {
			type: "string",
			required: true,
			description: "The @e ref to click."
		} },
		output: {
			schema: {
				type: "object",
			...
			render: (_args, value) => [{
				type: "text",
				text: `Clicked ${value.ref}`
			}]
		},
		async execute(args, _exec) {
			const cfg = current();
			await (await ensurePage(session, cfg)).click(refSelector(args.ref), { timeout: cfg.timeoutMs });
			return {
				action: "click",
				ref: args.ref
			};
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			ref: {
				type: "string",
				required: true,
				description: "The @e ref of the input."
			},
			text: {
				type: "string",
				required: true,
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: `Typed into ${value.ref}`
			}]
		},
		async execute(args, _exec) {
			await (await ensurePage(session, current())).fill(refSelector(args.ref), args.text);
			return {
				action: "type",
				ref: args.ref
			};
	...
	ctx.tools.register(defineTool({
		...
		parameters: { expression: {
			type: "string",
			required: true,
			description: "A JS expression; the page evaluates it and returns the value."
		} },
		output: {
			schema: {
				type: "object",
		...
		async execute(args, _exec) {
			const raw = await (await ensurePage(session, current())).evaluate(args.expression);
			let serialized;
			try {
				serialized = JSON.stringify(raw);
				if (serialized === void 0) serialized = String(raw);
			} catch {
...
function withTimeout(config, timeoutMs) {
	return timeoutMs === void 0 ? config : {
		...config,
		timeoutMs
	};
}
//#endregion
export { BROWSER_SETTINGS_NAMESPACE, Config, apply, inject, name };
